...
class Block(nn.Module):
    def __init__(
        ...
        self.attn_res_scale = nn.Parameter(torch.tensor(0.9, dtype=torch.float32))
        self.mlp_res_scale = nn.Parameter(torch.tensor(0.7, dtype=torch.float32))
        self.attn_scale = nn.Parameter(torch.full((dim,), 0.85, dtype=torch.float32))
        self.mlp_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))

    def forward(self, x: Tensor) -> Tensor:
        attn_res_scale = self.attn_res_scale.to(dtype=x.dtype)
        mlp_res_scale = self.mlp_res_scale.to(dtype=x.dtype)
...
def main() -> None:
    ...
    if master_process:
        ...
        for i, block in enumerate(base_model.blocks):
            log0(
                f"layer:{i} attn_res_scale:{block.attn_res_scale.item():.6f} "
                f"mlp_res_scale:{block.mlp_res_scale.item():.6f}"
            )
            log0(
