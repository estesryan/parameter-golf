...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, memory: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.memory_scale is not None and memory is not None:
            x[:, 0, :] = x[:, 0, :] + self.memory_scale.to(dtype=x.dtype)[None, :] * memory.to(dtype=x.dtype)[None, :]

        for block in self.blocks:
            x = block(x)

        x = self.final_norm(x)
        new_memory = x[:, -1, :].mean(dim=0).detach() if self.memory_scale is not None else None
