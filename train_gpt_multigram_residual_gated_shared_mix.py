"""
Multilag Bigram + Gated Shared-Basis Trigram Residual Transformer with Local/Transformer Mixing
...
contributions, and a learned per-position local-vs-transformer mixture gate + shallow U-Net
transformer with partial RoPE, tied embeddings, and bfloat16 training.
...
the model learns when to trust local logits and when to hand off to the transformer.
"""
...
class GPT(nn.Module):
    def __init__(
        ...
        _lag_w = torch.tensor([1.0, 0.12, 0.04], dtype=torch.float32)
        if num_lags != 3:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        gate_logits = (
            self.tri_gate_emb(t1).float()
            + 0.5 * self.tri_gate_emb(t2).float()
            + 0.25 * self.tri_gate_emb(t3).float()
        )
        ...
        z13 = self.tri_a[t1] * self.tri_b[t3]
        tri13 = z13 @ self.tri13_out

        z23 = self.tri_a[t2] * self.tri_b[t3]
        tri23 = z23 @ self.tri23_out

        tri12_mix = (self.tri12_w.float() * tri_gates[..., 0])[..., None]
        ...
        logits = (
            local_mix * local_logits
            + transformer_mix * (self.transformer_scale * transformer_logits)
        )
...
def main() -> None:
    ...
    scalar_params.extend([
        base_model.bigram_scale,
        base_model.transformer_scale,
    ...
    optimizer_bigram = torch.optim.Adam(
        [{"params": bigram_matrix_params, "lr": args.bigram_lr, "base_lr": args.bigram_lr}],
        betas=(args.beta1, args.beta2),
        eps=args.adam_eps,
        fused=True,
    ) if bigram_matrix_params else None
    optimizers: list[torch.optim.Optimizer] = [optimizer_tok, optimizer_muon, optimizer_scalar]
    if optimizer_bigram is not None:
    ...
    log0(f"bigram_trainable:{args.bigram_trainable} bigram_lr:{args.bigram_lr}")
    log0(
    ...
    while True:
        ...
        if args.adam_weight_decay > 0:
            with torch.no_grad():
                for opt in [optimizer_tok, optimizer_scalar, optimizer_bigram]:
                    if opt is None:
                        continue
                    for group in opt.param_groups:
                        lr = group["lr"]
