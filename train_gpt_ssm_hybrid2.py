...
class SSMBlock(nn.Module):
    def __init__(self, dim: int, state_dim: int, ssm_mlp_mult: int, ssm_module: nn.Module):
        super().__init__()
        self.norm = RMSNorm()
...
class GPT(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            if i == 2:
                ssm_module = ConvSSMBlock(model_dim, ssm_state_dim, conv_kernel=ssm_conv_kernel)
                blocks.append(SSMBlock(model_dim, ssm_state_dim, ssm_mlp_mult, ssm_module))
            elif i == 8:
                ssm_module = SSDSSMBlock(model_dim, ssm_state_dim)
                blocks.append(SSMBlock(model_dim, ssm_state_dim, ssm_mlp_mult, ssm_module))
            else:
                blocks.append(
                    Block(
