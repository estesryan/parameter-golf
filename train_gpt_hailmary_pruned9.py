"""
Bigram-Residual Transformer — parameter-golf submission (pruned9).

Combination of two independently validated ideas:
  1. Multi-lag low-rank bigram base (pruned8): sum of low-rank bigrams at lags [1,2,4,8,16]
  2. Context-dependent scalar gate over the base (pruned6): transformer-derived gate modulates base

Combined formula:
  logits = (1 + gate) * multilag_bigram_logits + residual_logits

where:
  - multilag_bigram_logits = Σ_i alpha_i * bigram_i(token_{t-lag_i})  (pruned8 base, unchanged)
  - residual_logits = transformer_scale * transformer_logits             (pruned8 transformer, unchanged)
  - gate = tanh(bigram_gate_proj(x_norm))                               (scalar per token, zero-init)

Gate is zero-initialized → exact pruned8 behavior at step 0. +448 params over pruned8.
...
Motivating data analysis:
  ...
  - Ablation shows strong short/mid-range dependence out to lag 128

Architecture:
  - Multi-lag low-rank bigram logit base: sum of rank-R approximations at lags [1,2,4,8,16].
    Each lag uses two [V×R] factor matrices; lag weights are learned scalars (init [1,0,0,0,0]).
  - Context-dependent scalar gate (USE_BIGRAM_GATE=1): nn.Linear(model_dim, 1, bias=False),
    zero-initialized, applied as (1 + tanh(gate)) * bigram_base.
  ...
  - Minimal RoPE: only ROPE_PARTIAL_DIMS (default 16) of head dims get positional encoding
...
been removed. The real model is: multi-lag low-rank bigram base + scalar gate + transformer residual.
"""
...
class Hyperparameters:
    ...
    use_bigram_gate = bool(int(os.environ.get("USE_BIGRAM_GATE", "1")))
...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        # Context-dependent gate that modulates bigram base strength.
        # gate → 0 at init (zero weights) → exact baseline behavior at step 0.
        if use_bigram_gate:
            self.bigram_gate_proj = nn.Linear(model_dim, 1, bias=False)
            self.bigram_gate_proj._zero_init = True
        else:
            self.bigram_gate_proj = None
    ...
    def _compute_logits(self, input_ids: Tensor) -> tuple[Tensor, Tensor]:
        ...
        if self.bigram_gate_proj is not None:
            gate = self.bigram_gate_proj(x_norm)          # [B, T, 1]
            gate = torch.tanh(gate).reshape(-1, 1)        # [B*T, 1]
            full_logits = (1.0 + gate) * bigram_flat + self.transformer_scale * transformer_logits
        else:
...
def main() -> None:
    ...
    log0(f"bigram_gate:{'enabled' if args.use_bigram_gate else 'disabled'}")
