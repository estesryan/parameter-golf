...
# -----------------------------
# PARALLEL LOCAL N-GRAM MIXER (Change 1)
# -----------------------------
# Bigram, trigram, and pointwise convolutions applied in parallel with learned
# per-channel gates, then merged residually into a single RMSNorm output.

class CausalConvEncoder(nn.Module):
    def __init__(self, dim: int, num_layers: int = 3):
        super().__init__()
        del num_layers  # accepted for API compatibility, not used
        self.conv_k2 = nn.Conv1d(dim, dim, kernel_size=2, groups=(dim // 4), bias=False)
        self.conv_k3 = nn.Conv1d(dim, dim, kernel_size=3, groups=(dim // 4), bias=False)
        self.conv_k1 = nn.Conv1d(dim, dim, kernel_size=1, groups=1, bias=False)
        self.norm = RMSNorm()
        self.gate2 = nn.Parameter(torch.ones(dim, dtype=torch.float32) * 0.1)
        self.gate3 = nn.Parameter(torch.ones(dim, dtype=torch.float32) * 0.1)
        self.gate1 = nn.Parameter(torch.ones(dim, dtype=torch.float32) * 0.1)

    def forward(self, x: Tensor) -> Tensor:
        # x: [B, T, D]
        xt = x.transpose(1, 2)  # → [B, D, T]
        x2 = self.conv_k2(F.pad(xt, (1, 0)))   # causal pad 1
        x3 = self.conv_k3(F.pad(xt, (2, 0)))   # causal pad 2
        x1 = self.conv_k1(xt)                   # no padding
        # gates: [D] → [1, D, 1] for broadcast over B and T
        g2 = self.gate2.to(x.dtype).view(1, -1, 1)
        g3 = self.gate3.to(x.dtype).view(1, -1, 1)
        g1 = self.gate1.to(x.dtype).view(1, -1, 1)
        merged = xt + g2 * x2 + g3 * x3 + g1 * x1  # [B, D, T]
        return self.norm(merged.transpose(1, 2))      # → [B, T, D]
