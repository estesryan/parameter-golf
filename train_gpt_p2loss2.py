    ...
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                ...
                batch_loss = m.forward_ce(x, y).detach()
...
class MLP(nn.Module):
    ...
    def __init__(self, dim: int, mlp_mult: int):
        ...
        self.proj._zero_init = True
        self.activation_type = os.environ.get("ACTIVATION_TYPE", "prelu2")
        if self.activation_type == "prelu2":
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, step: int | None = None, total_steps: int | None = None) -> Tensor:
        ...
        # --- P2 ---
        if loss_type == "p2":
            weights = (1.0 - p) ** 2  # strong focus on hard tokens
        ...
        return -(weights.detach() * target_logp).mean()

    def forward_ce(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        logits = self._logits(input_ids)
        targets = target_ids.reshape(-1)
        log_probs = F.log_softmax(logits.float() * self.logit_sharpen, dim=-1)
        target_logp = log_probs.gather(-1, targets.unsqueeze(-1)).squeeze(-1)
