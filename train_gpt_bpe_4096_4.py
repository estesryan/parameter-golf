"""
GPT training script using a 4096-vocab BPE tokenizer.
...
class Muon(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            # Reuse a persistent bf16 flat buffer across steps to avoid repeated allocations.
            # Recreate only if the size changes (should not happen in practice).
            if (
                "_updates_flat" not in group
                or group["_updates_flat"].numel() != total_params
                or group["_updates_flat"].device != params[0].device
            ):
                group["_updates_flat"] = torch.empty(total_params, device=params[0].device, dtype=torch.bfloat16)
...
def best_clip_per_row(t32: Tensor, qmax: int) -> Tensor:
    ...
    for pct in GPTQ_LITE_PERCENTILES:
        ...
        q = torch.round(clipped / scale[:, None]).clamp(-qmax, qmax)
        mse = (t32 - q * scale[:, None]).pow(2).mean(dim=1)
        if best_mse is None:
            best_clip, best_mse = clip, mse
        else:
            better = mse < best_mse
...
def quantize_float_tensor(name: str, t: Tensor) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim == 2:
        clip_abs = best_clip_per_row(t32, qmax) if t32.numel() else torch.empty((t32.shape[0],), dtype=torch.float32)
...
class TokenStream:
    ...
    def advance(self, n: int) -> None:
        # Skip forward n tokens without reading their values. Shard boundaries
        # are handled the same way as take() so stream position stays consistent.
        remaining = n
        while remaining > 0:
...
class DistributedTokenLoader:
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        # Rank-local partitioning at the stream level: each rank reads only its own
        # non-overlapping span of per_rank_span tokens from the global chunk.
        # Ranks before this one are skipped with advance() (no data copy), and ranks
        # after are skipped the same way, so every rank's stream advances identically
        # by per_rank_span * world_size and the global token order is preserved.
...
class CastedLinear(nn.Linear):
    # Keep weights in fp32 for optimizer/state quality; forward uses a cached bf16 copy
    # refreshed once after each optimizer step instead of casting every matmul.
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Initialized to None; populated by refresh_forward_weight() before first use.
        # persistent=False keeps it out of state_dict / serialization.
        self.register_buffer("weight_bf16", None, persistent=False)
    ...
    def refresh_forward_weight(self) -> None:
        ...
        if (
            ...
            or self.weight_bf16.device != w.device
        ):
            self.weight_bf16 = w.contiguous()
        else:
            self.weight_bf16.copy_(w)

    def forward(self, x: Tensor) -> Tensor:
        bias = self.bias.to(x.dtype) if self.bias is not None else None
        return F.linear(x, self.weight_bf16, bias)


@torch.no_grad()
def refresh_casted_linear_weights(module: nn.Module) -> None:
    """Refresh the cached bf16 forward weights for all CastedLinear submodules."""
    for m in module.modules():
        if isinstance(m, CastedLinear):
            m.refresh_forward_weight()
...
def main() -> None:
    ...
    refresh_casted_linear_weights(base_model)
