"""
Bigram-Residual Transformer — parameter-golf submission (pruned10_local1).

Structured residual language model: low-rank bigram base + compact transformer residual.
Single local mixing block replaces first decoder transformer block.
...
class LocalMixBlock(nn.Module):
    ...
    def __init__(self, dim: int):
        super().__init__()
        self.norm = RMSNorm()
        self.dw_conv = nn.Conv1d(dim, dim, kernel_size=5, padding=2, groups=dim, bias=False)
