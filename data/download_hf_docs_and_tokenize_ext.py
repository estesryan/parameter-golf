...
PYTHONUNBUFFERED=1 python3 data/download_hf_docs_and_tokenize_ext.py \
  --output-root ./data \
  --tokenizer-config ./data/tokenizer_specs_6144.json \
  --tokenizer-train-docs 1000000 \
  --train-shards 80

PYTHONUNBUFFERED=1 python3 data/download_hf_docs_and_tokenize_ext.py \
  --output-root ./data \
  --tokenizer-config ./data/tokenizer_specs_5120.json \
  --tokenizer-train-docs 1000000 \
  --train-shards 80
...
PYTHONUNBUFFERED=1 python3 data/download_hf_docs_and_tokenize_ext.py \
  --output-root ./data \
  --tokenizer-config ./data/tokenizer_specs_3072.json \
  --tokenizer-train-docs 1000000 \
  --train-shards 80

PYTHONUNBUFFERED=1 python3 data/download_hf_docs_and_tokenize_ext.py \
  --output-root ./data \
  --tokenizer-config ./data/tokenizer_specs_2048.json \
  --tokenizer-train-docs 1000000 \
  --train-shards 80
...
def export_shards(
    ...
    *,
    ...
    if not (stop_export and split == "train" and max_train_shards is not None and shards["train"] >= max_train_shards):
        flush()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=80,
        help="Limit number of training shards to export.",
    )
