...
Architecture:
  - Low-rank bigram logit model: initialized via truncated SVD of corpus bigram counts (rank R).
    ...
    Parameter savings: V² → 2·V·R (e.g. 16× reduction at R=32, V=1024).
  - Hashed trigram logit correction (USE_TRIGRAM_LOGIT_CORRECTION): explicit (prev2, prev1) context
    hashed into TRIGRAM_LOGIT_HASH_SIZE buckets; each bucket maps to a low-rank [R] latent via
    an embedding, then projected to vocab via trigram_vocab_factors [V, R]. This offloads short-range
    n-gram structure from the transformer into a compact logit-space table.
    logits = bigram_base + trigram_logit_correction + transformer_residual
  - Multi-hash n-gram input features: K independent hash functions (default K=3), each mapping
    (prev1, prev2) to a separate embedding table of size NGRAM_HASH_SIZE. Features are summed
    and scaled by a single learned scalar (init 0.05). Different multiplier constants per hash
    minimize collision overlap across tables.
  ...
  - Optional distillation (USE_DISTILLATION): CE loss on full logits; KL loss on residual logits only
    (residual = full_logits − bigram_base − trigram_correction, so all structured base is factored out)
...
class Hyperparameters:
    ...
    # --- Multi-hash n-gram: K independent hash tables summed at input ---
    ngram_num_hashes = int(os.environ.get("NGRAM_NUM_HASHES", 3))
    ngram_hash_size = int(os.environ.get("NGRAM_HASH_SIZE", 4096))

    # --- QAT mode: off | late | progressive ---
    qat_mode = os.environ.get("QAT_MODE", "off")
    qat_start_frac = float(os.environ.get("QAT_START_FRAC", 0.7))

    # --- Hashed trigram logit correction: explicit (prev2,prev1) logit-space correction ---
...
class GPT(nn.Module):
    def __init__(
        ...
        bigram_rank: int = 32,
        ngram_num_hashes: int = 3,
        ngram_hash_size: int = 4096,
        ...
        # Low-rank bigram logit model: prev_factors @ next_factors.T ≈ log P(next|prev).
        # Initialized from corpus SVD, frozen by default (BIGRAM_TRAINABLE=0).
        # Kept in fp32 for precision; cast to compute dtype in _compute_logits like CastedLinear.
        self.bigram_rank = bigram_rank
        self.bigram_prev_factors = nn.Parameter(torch.zeros(vocab_size, bigram_rank))
        self.bigram_next_factors = nn.Parameter(torch.zeros(vocab_size, bigram_rank))

        # Multi-hash n-gram input features: K independent hash tables, summed and scaled.
        # Different multiplier pairs per hash minimize cross-table collision overlap.
        _HASH_MULTS = [(1009, 9176), (2654, 6571), (7919, 3299), (4231, 8573), (1637, 5419)]
        self.ngram_num_hashes = ngram_num_hashes
        self.ngram_hash_size = ngram_hash_size
        self.ngram_hash_mults: list[tuple[int, int]] = _HASH_MULTS[:ngram_num_hashes]
        self.ngram_embs = nn.ModuleList(
            [nn.Embedding(ngram_hash_size, model_dim) for _ in range(ngram_num_hashes)]
        )
        self.ngram_scale = nn.Parameter(torch.tensor(0.05, dtype=torch.float32))
        for emb in self.ngram_embs:
            nn.init.normal_(emb.weight, mean=0.0, std=0.02)

        # Hashed trigram logit correction: (prev2, prev1) → bucket → low-rank latent → vocab logits.
        # trigram_ctx_factors [HASH_SIZE, R]: context bucket embeddings.
        # trigram_vocab_factors [V, R]: shared vocab projection.
        # correction = trigram_logit_scale * (ctx_emb @ vocab_factors.T)
        self.use_trigram_logit = use_trigram_logit
        self.trigram_logit_hash_size = trigram_logit_hash_size
        self.trigram_logit_rank = trigram_logit_rank
        if use_trigram_logit:
            self.trigram_ctx_factors = nn.Embedding(trigram_logit_hash_size, trigram_logit_rank)
    ...
    def _compute_logits(self, input_ids: Tensor) -> tuple[Tensor, Tensor, Tensor | None]:
        """Shared forward computation.

        Returns (full_logits, bigram_base_logits, trigram_correction), all [B*T, V].
        trigram_correction is None when use_trigram_logit=False.
        structured_base = bigram_base + trigram_correction (or just bigram_base when disabled).
        transformer_residual = full_logits - structured_base.
        """
        ...
        # Multi-hash n-gram: sum K independent embedding lookups, each with distinct multipliers.
        ngram_feat = torch.zeros_like(tok)
        for emb, (m1, m2) in zip(self.ngram_embs, self.ngram_hash_mults):
            h = (prev1.to(torch.int64) * m1 + prev2.to(torch.int64) * m2) % self.ngram_hash_size
            ngram_feat = ngram_feat + emb(h)
        ...
        # Low-rank bigram base: prev_factors[t] @ next_factors.T — never materializes full [V×V].
        ...
        # Hashed trigram logit correction: (prev2, prev1) → bucket → low-rank latent @ vocab factors.
        if self.use_trigram_logit:
        ...
        full_logits = (
            bigram_flat
            + (trigram_correction if trigram_correction is not None else 0)
            + self.transformer_scale * transformer_logits
        )
        return full_logits, bigram_flat, trigram_correction  # [B*T, V], [B*T, V], [B*T, V] | None

    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        full_logits, bigram_flat, trigram_correction = self._compute_logits(input_ids)
        targets = target_ids.reshape(-1)

        if self._teacher is not None:
            # Residual distillation: CE on full logits, KL on transformer residuals only.
            # structured_base = bigram_base + trigram_correction factors out all explicit n-gram structure.
            # Each model's residual = full_logits - its own structured_base.
            T = self._distill_temperature
            ce = F.cross_entropy(full_logits.float() * self.logit_sharpen, targets)
            with torch.no_grad():
                t_full, t_bigram, t_trigram = self._teacher._compute_logits(input_ids)
                t_structured = t_bigram + (t_trigram if t_trigram is not None else 0)
                t_residual = (t_full - t_structured).float()
            s_structured = bigram_flat + (trigram_correction if trigram_correction is not None else 0)
            s_residual = (full_logits - s_structured).float()
...
def main() -> None:
    ...
    base_model.bigram_prev_factors.data.copy_(prev_factors.float())
    base_model.bigram_next_factors.data.copy_(next_factors.float())
    ...
    if args.use_distillation and args.distill_teacher_path:
        ...
        teacher_model = GPT(
            ...
            ngram_num_hashes=args.ngram_num_hashes, ngram_hash_size=args.ngram_hash_size,
            use_trigram_logit=args.use_trigram_logit,
            trigram_logit_hash_size=args.trigram_logit_hash_size,
            trigram_logit_rank=args.trigram_logit_rank,
        ).to(device).bfloat16()
    ...
    while True:
        ...
        if args.bigram_trainable and args.bigram_reg_lambda > 0:
            with torch.no_grad():
                base_model.bigram_prev_factors.data.sub_(
                    args.bigram_reg_lambda * (base_model.bigram_prev_factors.data - bigram_init_prev)
                )
                base_model.bigram_next_factors.data.sub_(
                    args.bigram_reg_lambda * (base_model.bigram_next_factors.data - bigram_init_next)
                )
    ...
    if master_process:
        ...
        _n_ngram = sum(e.weight.numel() for e in base_model.ngram_embs)
        _n_tok = base_model.tok_emb.weight.numel()
        _n_trigram_logit = (
            base_model.trigram_ctx_factors.weight.numel()
            + base_model.trigram_vocab_factors.numel()
            + base_model.trigram_logit_scale.numel()
        ) if base_model.use_trigram_logit else 0
        _trigram_skip = ("bigram_", "ngram_embs", "tok_emb", "trigram_")
        _n_transformer = sum(
            p.numel() for name, p in base_model.named_parameters()
            if not any(k in name for k in _trigram_skip)
        )
        ...
        log0(
            ...
            f" ngram_hashes={_n_ngram:,} ({args.ngram_num_hashes}×{args.ngram_hash_size})"
            f" trigram_logit={_n_trigram_logit:,}"
            f" (hash={args.trigram_logit_hash_size} rank={args.trigram_logit_rank} enabled={args.use_trigram_logit})"
