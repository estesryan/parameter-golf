...
VAL_STRIDE = 64  # sliding-window stride for validation
...
def eval_val(
    ...
    # Uses a sliding window (stride=VAL_STRIDE) so each token is evaluated with
    # full left context up to train_seq_len. Only the last VAL_STRIDE positions of
    # each window contribute to the loss, except the first window which contributes all.
    ...
    all_window_starts = list(range(0, val_tokens.numel() - seq_len, stride))
    num_windows = len(all_window_starts)
    win_start = (num_windows * rank) // world_size
    win_end = (num_windows * (rank + 1)) // world_size
    my_window_starts = all_window_starts[win_start:win_end]
    ...
    with torch.inference_mode():
        for batch_idx in range(0, len(my_window_starts), local_batch_seqs):
            batch_starts = my_window_starts[batch_idx : batch_idx + local_batch_seqs]
            B = len(batch_starts)
            x = torch.stack([val_tokens[s : s + seq_len] for s in batch_starts]).to(
                device=device, dtype=torch.int64, non_blocking=True
            )
            y = torch.stack([val_tokens[s + 1 : s + seq_len + 1] for s in batch_starts]).to(
                device=device, dtype=torch.int64, non_blocking=True
            )
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                token_losses = model(x, y, reduction="none").detach().reshape(B, seq_len)
            # Count only "new" tokens: the last stride positions per window.
            # The first global window counts all seq_len positions (no prior context).
            mask = torch.zeros(B, seq_len, dtype=torch.bool, device=device)
            mask[:, seq_len - stride :] = True
            if win_start + batch_idx == 0:
                mask[0, :] = True
            val_loss_sum += (token_losses * mask).to(torch.float64).sum()
            val_token_count += mask.sum()
            flat_mask = mask.reshape(-1)
            flat_y = y.reshape(-1)[flat_mask]
            flat_x = x.reshape(-1)[flat_mask]
            token_bytes = base_bytes_lut[flat_y].to(dtype=torch.int16)
            token_bytes += (has_leading_space_lut[flat_y] & ~is_boundary_token_lut[flat_x]).to(dtype=torch.int16)
...
def main() -> None:
    ...
    if USE_ZSTD:
        ...
        quant_raw = zlib.decompress(quant_blob_disk)
