...
class GPT(nn.Module):
    def __init__(
        ...
        self.trigram_embed2 = nn.Embedding(8192, vocab_size)
        self.trigram_scale2 = nn.Parameter(torch.tensor(0.10, dtype=torch.float32))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        trigram_hash2 = (
            ((prev2.to(torch.int64) * 2246822519) ^ (prev1.to(torch.int64) * 3266489917))
            & 8191
        ).to(torch.int64)

        trigram_logits2 = self.trigram_embed2(trigram_hash2).reshape(-1, self.tok_emb.num_embeddings)
        ...
        logits = (
            ...
            + self.trigram_scale2 * trigram_logits2.float()
        )
...
def main() -> None:
    ...
    trigram2_named_params = list(base_model.trigram_embed2.named_parameters())
    matrix_params = [
        p
    ...
    matrix_params.extend(
        p
        for name, p in trigram2_named_params
        if p.ndim == 2 and not any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)
    )
    scalar_params.extend(
        p
    ...
    scalar_params.append(base_model.trigram_scale2)
