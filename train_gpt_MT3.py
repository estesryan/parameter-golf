...
class Hyperparameters:
    ...
    trigram_num_buckets = int(os.environ.get("TRIGRAM_NUM_BUCKETS", "4096"))
...
class TrigramHead(nn.Module):
    """
    Maps (t-2, t-1) context to a logit correction via hashed bucket lookup.

    Architecture:
        hash(t-2, t-1) -> bucket_id
        bucket_emb[bucket_id] -> [rank]
        out_proj([rank]) -> [vocab_size]

    Added directly to transformer logits before softmax.
    """
    def __init__(self, vocab_size: int, num_buckets: int, rank: int):
        super().__init__()
    ...
    def forward(self, input_ids: Tensor) -> Tensor:
        ...
        t_minus_1 = F.pad(input_ids, (1, 0))[:, :seqlen]
        t_minus_2 = F.pad(input_ids, (2, 0))[:, :seqlen]

        # Stronger hash (better mixing, fewer collisions)
        bucket = ((t_minus_2.long() * 1315423911) ^ (t_minus_1.long() * 2654435761)) % self.num_buckets

        emb = self.bucket_emb(bucket)              # [B, T, rank]
        logits = self.out_proj(emb)                # [B, T, vocab]
        return logits.reshape(-1, logits.size(-1))  # [B*T, vocab]
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        trigram_num_buckets=args.trigram_num_buckets,
    ...
    matrix_params.append(base_model.trigram_head.out_proj.weight)
    matrix_params.append(base_model.trigram_head.bucket_emb.weight)
    ...
    log0(f"markov_transformer:trigram_num_buckets:{args.trigram_num_buckets} trigram_rank:{args.trigram_rank} rope_partial_dims:{args.rope_partial_dims} encoder_layer_frac:{args.encoder_layer_frac} enc_layers:{n_enc} dec_layers:{n_dec}")
